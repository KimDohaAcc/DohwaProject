...
export const useUserStore = defineStore('user', () => {
  ...
  const submitNewUser = async (newUser) => {
    try {
      ...
      const response = await axiosInstance.post(`${dApi}/user`, newUser);
      alert("회원가입이 완료되었습니다.");
      router.push('/');
    } catch (error) {
  ...
  const updateUser = function () {
    axiosInstanceWithToken
      .put(`${dApi}/auth/user`, loginUser.value)
      .then((res) => {
        console.log("업데이트 완료")
      })
  }
